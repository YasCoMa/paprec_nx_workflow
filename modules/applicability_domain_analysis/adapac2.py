...
class ApplicabilityDomainDetector(BaseEstimator, TransformerMixin):
    def __init__(self, k=5, alpha=0.95, metric='euclidean', scaler=StandardScaler()):
        """
        ...
        metric : string, optional
            one of the metrics accepted by scipy cdist function: 'braycurtis', 'canberra', 'chebyshev', 'cityblock', 'correlation', 'cosine', 'dice', 'euclidean', 'hamming', 'jaccard', 'jensenshannon', 'kulczynski1', 'mahalanobis', 'matching', 'minkowski', 'rogerstanimoto', 'russellrao', 'seuclidean', 'sokalmichener', 'sokalsneath', 'sqeuclidean', 'yule', by default euclidean
    ...
    def _get_distance(self, XA, XB, sort = True):
        distance = cdist(XA, XB, metric = self.metric)    # ユークリッド距離を行ベクトルごとに算出する．distance.shape[0] == len(XA), distance.shape[1] == len(XB)
        if sort:
            distance.sort()
        return distance
